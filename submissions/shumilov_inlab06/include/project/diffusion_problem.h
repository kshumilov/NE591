...
struct Space2D {
    ...
    constexpr Space2D(const scalar_t a_, const scalar_t b_)
        : X{a_}
        , Y{b_} { validate(); }
...
struct GridPoints2D {
    ...
    constexpr GridPoints2D(const int M_, const int N_)
        : NX{M_}
        , NY{N_} { validate(); }
...
struct Grid2D {
    ...
    constexpr Grid2D(const Space2D<scalar_t>& space_, const GridPoints2D& points_)
        : space{space_}
      , points{points_} {
        validate();
    }
...
struct IsotropicSteadyStateDiffusion2D {
    ...
    [[nodiscard]] constexpr auto operator_element(const std::size_t I, const std::size_t J) const -> scalar_t {
        ...
        if (((0U < i && i - 1U == k) || (i < M_ - 1U && i + 1U == k)) && j == l)
            return -D_over_delta_sq;

        if (i == k && ((0U < j && j - 1U == l) || (j < N_ - 1U && j + 1U == l)))
            return -D_over_gamma_sq;

        if (i == k && j == l)
            return 2.0 * (D_over_delta_sq + D_over_gamma_sq) + absorption_scattering;
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(
        IsotropicSteadyStateDiffusion2D<scalar_t>
      , grid
      , diffusion_coefficient
      , absorption_scattering
      , source
