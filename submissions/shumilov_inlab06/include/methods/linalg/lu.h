...
constexpr void lu_factor_inplace_update(Matrix<scalar_t>& A, const std::size_t k, LUResult& result)
{
...
template<std::floating_point scalar_t>
constexpr LUResult lu_factor_inplace(Matrix<scalar_t>& A)
{
    ...
    for (std::size_t k{0U}; k < n - 1U; ++k) {
        lu_factor_inplace_update(A, k, result);
    }
...
template<std::floating_point scalar_t>
[[nodiscard]] constexpr
std::pair<Matrix<scalar_t>, LUResult> lup_factor_inplace(Matrix<scalar_t>& A)
{
    const auto n{std::min(A.rows(), A.cols())};
