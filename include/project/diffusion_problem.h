...
template<std::floating_point scalar_t> struct Space2D {
    ...
    constexpr Space2D(const scalar_t a_, const scalar_t b_) : X{a_}, Y{b_} { validate(); }
...
struct GridPoints2D {
    ...
    constexpr GridPoints2D(const int M_, const int N_) : NX{M_}, NY{N_} { validate(); }
...
template<std::floating_point scalar_t> struct Grid2D {
...
template<std::floating_point DType> struct IsotropicSteadyStateDiffusion2D {
    Grid2D<DType> grid{};
    ...
    [[nodiscard]] constexpr auto operator_element(const std::size_t I, const std::size_t J) const -> DType {
        ...
        const auto D_over_gamma_sq = diffusion_coefficient / (grid.dy() * grid.dy());

        const auto M = static_cast<std::size_t>(grid.points.NX);
        const auto N = static_cast<std::size_t>(grid.points.NY);
        ...
        if (j_phi == j_q && ((0U < i_q && i_q == i_phi + 1U) || (i_q + 1U < M && i_q + 1U == i_phi)))
            return -D_over_delta_sq;

        // -D(\phi{j + 1, i} + \phi{j - 1, i}) / \gamma ^ 2
        if (i_phi == i_q && ((0U < j_q && j_q == j_phi + 1U) || (j_q + 1U < N && j_q + 1U == j_phi)))
            return -D_over_gamma_sq;

        if (i_phi == i_q && j_phi == j_q)
            return DType{2.0} * (D_over_delta_sq + D_over_gamma_sq) + absorption_scattering;
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(
            IsotropicSteadyStateDiffusion2D<DType>, grid, diffusion_coefficient, absorption_scattering, source)
};
