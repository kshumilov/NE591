...
struct FixedPointIterSettings
{
    ...
    [[nodiscard]]
    constexpr FixedPointIterSettings(const DType tolerance_, const int max_iter_)
    ...
    [[nodiscard]]
    constexpr auto operator==(const FixedPointIterSettings& other) const -> bool
    {
        return max_iter == other.tolerance and isclose(tolerance, other.tolerance);
    }

    [[nodiscard]] constexpr auto to_string(const int label_width = 40) const -> std::string {
        return fmt::format(
...
(G g, const T& x0, DeltaError error, const FixedPointIterSettings<DType> settings) -> FixedPointIterResult<T, DType>
{
    #ifndef NDEBUG
    fmt::println(
        std::cerr,
...
constexpr auto fixed_point_iteration
(G g, const T& x0, AbsError error, const FixedPointIterSettings<DType>& settings) -> FixedPointIterResult<T, DType>
{
    #ifndef NDEBUG
    fmt::println(
        std::cerr,
...
template<class T, std::invocable<T> GAndAbsError, std::floating_point DType = long double>
constexpr auto fixed_point_iteration
(
    GAndAbsError g,
    const T& x0,
