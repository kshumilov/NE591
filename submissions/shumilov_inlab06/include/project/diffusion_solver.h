...
struct LUPSolver {
  ...
  template <class T>
  [[nodiscard]] Solution<T>
  solve(const IsotropicSteadyStateDiffusion2D<T> &problem) const {
    problem.validate();
    ...
    if (lu_result == LUResult::SmallPivotEncountered) {
      std::cerr << fmt::format(fmt::emphasis::bold | fg(fmt::color::red),
    ...
    return Solution<T>{.problem = problem,
  ...
  build_operator(const IsotropicSteadyStateDiffusion2D<T> &problem) const
      -> Matrix<T> {
    const auto dim = problem.grid.points.size();

    return Matrix<T>::from_func(static_cast<std::size_t>(dim),
  ...
  template <class T>
  [[nodiscard]] constexpr auto
  build_rhs(const IsotropicSteadyStateDiffusion2D<T> &problem) const
      -> std::span<const T> {
    return problem.source.data();
  }

  template <class T>
  [[nodiscard]] constexpr auto calculate_residual(const Matrix<T> &LU,
                                                  std::span<const T> x,
