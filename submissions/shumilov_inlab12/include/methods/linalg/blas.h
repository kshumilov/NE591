...
template<std::floating_point T>
constexpr auto dot(const std::vector<T>& lhs, const std::vector<T>& rhs)
{
...
template<std::floating_point T>
constexpr auto norm_l2(const std::vector<T>& v) { return std::sqrt(dot<T>(v, v)); }

template<std::floating_point T>
constexpr auto norm_linf(const std::vector<T>& v)
{
    return std::transform_reduce(
        std::cbegin(v),
        std::cend(v),
