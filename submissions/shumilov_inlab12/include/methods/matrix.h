...
class Matrix
{
    public:
        ...
        [[nodiscard]] static constexpr auto eye(const idx_t rows) -> Matrix
        {
            return Matrix::eye(rows, rows);
        }

        [[nodiscard]]
        static constexpr auto diagonal(const std::vector<scalar_t>& diagonal)
        {
            std::vector<scalar_t> data(diagonal.size() * diagonal.size(), scalar_t{});
            for (std::size_t i = 0; i < diagonal.size(); ++i)
            {
                data[ravel2d(i, i, diagonal.size())] = diagonal[i];
            }

            return Matrix{ diagonal.size(), diagonal.size(), std::move(data) };
        }
