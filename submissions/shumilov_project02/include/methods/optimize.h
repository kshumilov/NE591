...
template<std::floating_point DType = long double> struct FixedPointIterSettings {
    ...
    [[nodiscard]] constexpr FixedPointIterSettings(const DType tolerance_, const int max_iter_)
        : tolerance(tolerance_), max_iter(max_iter_) {
        if (max_iter <= int{}) {
    ...
    [[nodiscard]] constexpr auto operator==(const FixedPointIterSettings &other) const -> bool {
...
template<class T, std::floating_point DType = long double> struct FixedPointIterResult {
    T x{};
...
constexpr auto fixed_point_iteration(G g, const T &x0, DeltaError error, const FixedPointIterSettings<DType> settings)
    ...
    for (int i = 0; i < settings.max_iter; ++i) {
        ...
        if (current_error < settings.tolerance) {
            return FixedPointIterResult<T, DType>{
                    .x = x_next, .converged = true, .iters = i + 1, .error = current_error};
        }
    ...
    return FixedPointIterResult<T, DType>{
            .x = x_curr, .converged = false, .iters = settings.max_iter, .error = current_error};
}


template<class T, std::invocable<T> G, std::invocable<T> AbsError, std::floating_point DType = long double>
constexpr auto fixed_point_iteration(G g, const T &x0, AbsError error, const FixedPointIterSettings<DType> settings)
        -> FixedPointIterResult<T, DType> {
#ifndef NDEBUG
    fmt::println(
            std::cerr,
    ...
    return FixedPointIterResult<T, DType>{
            .x = x, .converged = false, .iters = settings.max_iter, .error = current_error};
}
#endif // OPTIMIZE_H
