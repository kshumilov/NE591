...
struct FixedPointIterSettings {
    ...
    [[nodiscard]] constexpr
    explicit FixedPointIterSettings(const DType tolerance_, const int max_iter_)
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(FixedPointIterSettings<DType>, max_iter, tolerance)
};
 ...
 */
template<class T, std::invocable<T> G, std::invocable<T, T> Error, std::floating_point DType = long double>
constexpr auto fixed_point_iteration(
    G g, const T& x0, Error error,
    const FixedPointIterSettings<DType> settings
) -> FixedPointIterResult<T>
{
    #ifndef NDEBUG
    fmt::println(
        std::cerr,
