...
auto get_local_rank(const MPI_Comm comm) -> int {
    ...
    if (const auto error_code = MPI_Comm_rank(comm, &rank);
        ...
        throw std::runtime_error(fmt::format(
            "MPI_Comm_rank failed: {}", error_code
        ));
    }
    return rank;
}
...
auto get_world_size(const MPI_Comm comm) -> int {
    ...
    if (const auto error_code = MPI_Comm_size(comm, &world_size);
        MPI_SUCCESS != error_code) {
        throw std::runtime_error(fmt::format(
            "MPI_Comm_size failed: {}", error_code
        ));
    }
    return world_size;
}
...
auto get_processor_name() -> std::string {
    ...
    if (const auto error_code = MPI_Get_processor_name(name.get(), &size);
        MPI_SUCCESS != error_code) {
        throw std::runtime_error(fmt::format(
            "MPI_Get_processor_name failed: {}", error_code
        ));
    }
...
auto hello_world(std::ostream& out) -> void {
    ...
    fmt::println(out, "Hello {:s} from MPI Task {:d} on host {:s}", name, local_rank, hostname);
}
...
auto main(int argc, char* argv[]) -> int {
    ...
    try {
        ...
        if (not output.is_open()) {
            throw std::runtime_error(fmt::format("Could not open file {}", output_filename));
        }
