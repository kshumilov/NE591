...
struct LUPSolver {
  ...
  /***
  *  @brief Solves Isotrpoic Steady state diffusion problem using LUP Factorization
  *
  *  @param Diffusion problem that implements operator_element function
  *
  *  @return Solution type that stores scalar flux and residual
  */
  template<class T>
  ...
  /**
   * @brief Builds operator A by element from a given Diffusion Problem
   * @param Diffusion problem
   * @return Matrix of opertor A
   */
  template<class T>
  [[nodiscard]] constexpr
